...
/**
 * Registers {@link Route}-annotated classes as Spring beans even without {@code @Component}.
 */
final class RouteComponentAutoRegistrar implements BeanDefinitionRegistryPostProcessor, BeanFactoryAware, ResourceLoaderAware, EnvironmentAware {
    private static final Logger log = LoggerFactory.getLogger(RouteComponentAutoRegistrar.class);
    ...
    public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry) throws BeansException {
        if (beanFactory == null) {
            log.debug("Skipping @Route component auto-registration: beanFactory unavailable");
            return;
        }

        Set<String> basePackages = resolveBasePackages(registry);
        if (basePackages.isEmpty()) {
            log.debug("Skipping @Route component auto-registration: no base packages resolved");
            return;
        }
        ...
        for (String basePackage : basePackages) {
            for (BeanDefinition candidate : scanner.findCandidateComponents(basePackage)) {
                ...
                log.debug("Registered @Route class as bean: {} -> {}", className, beanName);
            }
        }
    }
