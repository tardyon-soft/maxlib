...
class QuarkusRouteAutoRegisterTest {
    @Inject
    Dispatcher dispatcher;

    @Test
    void autoRegisterRouteBeanIsRegistered() {
    ...
    @Singleton
    static final class AutoRouteConfig {
        static final AtomicInteger CALLS = new AtomicInteger();

        @Produces
        AutoRouteController autoRoute() {
            return new AutoRouteController();
        }

        @Route(value = "auto", autoRegister = true)
        static class AutoRouteController {
            @Command("start")
