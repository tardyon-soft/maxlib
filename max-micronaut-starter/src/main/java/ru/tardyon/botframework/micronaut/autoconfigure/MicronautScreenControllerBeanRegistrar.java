...
/**
 * Discovers {@link ScreenController}-annotated classes even without explicit bean declaration.
 */
@Singleton
public final class MicronautScreenControllerBeanRegistrar {
    private final ApplicationContext applicationContext;

    public MicronautScreenControllerBeanRegistrar(ApplicationContext applicationContext) {
        this.applicationContext = applicationContext;
    }

    public List<Object> discoverScreenControllerInstances() {
        LinkedHashSet<Object> instances = new LinkedHashSet<>();
        Set<Class<?>> knownBeanTypes = new LinkedHashSet<>();
        applicationContext.getBeanDefinitions(Object.class).forEach(definition -> knownBeanTypes.add(definition.getBeanType()));

        LinkedHashSet<String> basePackages = new LinkedHashSet<>();
        for (Class<?> beanType : knownBeanTypes) {
        ...
        for (String basePackage : basePackages) {
            for (Class<?> candidate : scanClasses(basePackage)) {
                ScreenController controller = candidate.getAnnotation(ScreenController.class);
    ...
    private void loadIfCandidate(String className, List<Class<?>> classes) {
        try {
            ...
            if (candidate.isAnnotationPresent(ScreenController.class)
    ...
    private Object instantiate(Class<?> type) {
        try {
            ...
            throw new IllegalStateException("Failed to instantiate autodetected @ScreenController class: " + type.getName(), e);
        }
    }
}
